...
class EditorViewController: UIViewController {
    ...
    func addButtonForView() {
        getSlider.updateLayerFrames()
        getPlayer.slider.currentThumbPoint.x = CGFloat(getPlayer.slider.lowerValue)
        let value = Float(endTimeOutput) - getPlayer.outputValue
        print("\(value)value")
        let wPerSec = Double(viewSlider.frame.width) / Double(getPlayer.mediaDurationOut)
        print("\(wPerSec)wPerSec")
        let xPerSec = Float(wPerSec) * getPlayer.outputValue
        print("\(xPerSec)xPerSec")
        let startPoint = Double(viewSlider.frame.minX)
        let button = UIButton(frame: CGRect(x: CGFloat(startPoint) + CGFloat(xPerSec), y: viewSlider.center.y - 25, width: (CGFloat(value) * CGFloat(wPerSec)), height: getPlayer.slider.frame.height - 10))
...
extension EditorViewController: AddSubDelegate {
    ...
    func dataDelegate(sub: String, startTime: Int, endTime: Int, color: UIColor) {
        ...
        //MARK: Create array of range from start subtitle to end with gap by 0.001
        let doublesArray = Array(stride(from: start, to: end, by: 0.001))
